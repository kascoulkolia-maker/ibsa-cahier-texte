...
## ✅ Étape 6 — Accès public pour l'espace parents (QR code) ⚠️ OBLIGATOIRE

> **C'est cette étape qui permet aux parents de voir les séances en scannant le QR code.**  
> Sans ce SQL, la page parents affiche « Aucune séance disponible » même si des séances existent.

Ouvrez **SQL Editor** dans Supabase et collez ce bloc :

```sql
-- ═══════════════════════════════════════════════════════════════
-- VUE PUBLIQUE : séances visibles par les parents (sans compte)
-- ═══════════════════════════════════════════════════════════════

-- 1. Créer la vue seances_parents (colonnes publiques uniquement)
CREATE OR REPLACE VIEW public.seances_parents AS
SELECT
  s.id,
  s.classe,
  s.matiere,
  ...
  s.devoirs,
  s.files,
  u.login AS enseignant_nom
FROM public.seances s
LEFT JOIN public.users u ON u.id = s.teacher_id;

-- 2. Autoriser la lecture anonyme sur la vue
GRANT SELECT ON public.seances_parents TO anon;
GRANT SELECT ON public.seances_parents TO authenticated;

-- 3. Autoriser la lecture anonyme sur la table seances
--    (nécessaire car la vue interroge la table sous-jacente)
CREATE POLICY "seances_anon_read_parents" ON public.seances
  FOR SELECT TO anon
  USING (true);

-- 4. Autoriser la lecture anonyme sur users (pour le nom de l'enseignant)
--    Seul le login est exposé via la vue — les données sensibles restent protégées
CREATE POLICY "users_anon_read_login" ON public.users
  FOR SELECT TO anon
  USING (true);
```

Cliquez sur **Run**. Vous devriez voir `Success. No rows returned.`

> **Note sécurité** : La politique `seances_anon_read_parents` expose les séances à toute personne  
> qui connaît l'URL. C'est intentionnel — les parents n'ont pas de compte IBSA.  
> Les champs sensibles (`signales`, `observations`) ne sont **pas** inclus dans la vue.

---

## ✅ Étape 7 — Déploiement sur Netlify
...
### Les parents voient « Aucune séance disponible » après scan du QR code
- L'étape 6 (accès public) n'a pas été exécutée dans le SQL Editor → relancez le SQL de l'Étape 6
- Vérifiez que la vue `seances_parents` existe : **Table Editor** → chercher `seances_parents`
- Vérifiez que la politique `seances_anon_read_parents` existe : **Authentication → Policies → seances**
- Vérifiez l'URL du QR code : elle doit contenir `?classe=NOM_CLASSE` (ex: `?classe=6ème A`)
